"""add social_account

Revision ID: 0c62fa30dc0b
Revises: 7b480ce3b6af
Create Date: 2022-11-03 11:29:45.320527

"""
from alembic import op
import sqlalchemy as sa
from sqlalchemy.dialects import postgresql

# revision identifiers, used by Alembic.
revision = '0c62fa30dc0b'
down_revision = '7b480ce3b6af'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('social_account',
    sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('create_at', sa.TIMESTAMP(), nullable=False),
    sa.Column('updated_at', sa.TIMESTAMP(), nullable=False),
    sa.Column('is_deleted', sa.Boolean(), nullable=False),
    sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('social_name', sa.String(), nullable=False),
    sa.Column('social_id', sa.String(), nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['auth.users.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id'),
    sa.UniqueConstraint('social_id', 'social_name'),
    schema='auth'
    )
    op.create_unique_constraint(None, 'allowed_device', ['id'], schema='auth')
    op.drop_constraint('allowed_device_user_id_fkey', 'allowed_device', type_='foreignkey')
    op.create_foreign_key(None, 'allowed_device', 'users', ['user_id'], ['id'], source_schema='auth', referent_schema='auth')
    op.create_unique_constraint(None, 'permissions', ['id'], schema='auth')
    op.create_unique_constraint(None, 'roles', ['id'], schema='auth')
    op.create_unique_constraint(None, 'roles_permissions', ['id'], schema='auth')
    op.drop_constraint('roles_permissions_perm_id_fkey', 'roles_permissions', type_='foreignkey')
    op.drop_constraint('roles_permissions_role_id_fkey', 'roles_permissions', type_='foreignkey')
    op.create_foreign_key(None, 'roles_permissions', 'roles', ['role_id'], ['id'], source_schema='auth', referent_schema='auth')
    op.create_foreign_key(None, 'roles_permissions', 'permissions', ['perm_id'], ['id'], source_schema='auth', referent_schema='auth')
    op.create_unique_constraint(None, 'sessions', ['id'], schema='auth')
    op.drop_constraint('sessions_device_id_fkey', 'sessions', type_='foreignkey')
    op.drop_constraint('sessions_user_id_fkey', 'sessions', type_='foreignkey')
    op.create_foreign_key(None, 'sessions', 'users', ['user_id'], ['id'], source_schema='auth', referent_schema='auth')
    op.create_foreign_key(None, 'sessions', 'allowed_device', ['device_id'], ['id'], source_schema='auth', referent_schema='auth')
    op.create_unique_constraint(None, 'user_info', ['id'], schema='auth')
    op.drop_constraint('user_info_user_id_fkey', 'user_info', type_='foreignkey')
    op.create_foreign_key(None, 'user_info', 'users', ['user_id'], ['id'], source_schema='auth', referent_schema='auth')
    op.create_unique_constraint(None, 'users', ['id'], schema='auth')
    op.create_unique_constraint(None, 'users_roles', ['id'], schema='auth')
    op.drop_constraint('users_roles_role_id_fkey', 'users_roles', type_='foreignkey')
    op.drop_constraint('users_roles_user_id_fkey', 'users_roles', type_='foreignkey')
    op.create_foreign_key(None, 'users_roles', 'roles', ['role_id'], ['id'], source_schema='auth', referent_schema='auth')
    op.create_foreign_key(None, 'users_roles', 'users', ['user_id'], ['id'], source_schema='auth', referent_schema='auth')
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, 'users_roles', schema='auth', type_='foreignkey')
    op.drop_constraint(None, 'users_roles', schema='auth', type_='foreignkey')
    op.create_foreign_key('users_roles_user_id_fkey', 'users_roles', 'users', ['user_id'], ['id'])
    op.create_foreign_key('users_roles_role_id_fkey', 'users_roles', 'roles', ['role_id'], ['id'])
    op.drop_constraint(None, 'users_roles', schema='auth', type_='unique')
    op.drop_constraint(None, 'users', schema='auth', type_='unique')
    op.drop_constraint(None, 'user_info', schema='auth', type_='foreignkey')
    op.create_foreign_key('user_info_user_id_fkey', 'user_info', 'users', ['user_id'], ['id'])
    op.drop_constraint(None, 'user_info', schema='auth', type_='unique')
    op.drop_constraint(None, 'sessions', schema='auth', type_='foreignkey')
    op.drop_constraint(None, 'sessions', schema='auth', type_='foreignkey')
    op.create_foreign_key('sessions_user_id_fkey', 'sessions', 'users', ['user_id'], ['id'])
    op.create_foreign_key('sessions_device_id_fkey', 'sessions', 'allowed_device', ['device_id'], ['id'])
    op.drop_constraint(None, 'sessions', schema='auth', type_='unique')
    op.drop_constraint(None, 'roles_permissions', schema='auth', type_='foreignkey')
    op.drop_constraint(None, 'roles_permissions', schema='auth', type_='foreignkey')
    op.create_foreign_key('roles_permissions_role_id_fkey', 'roles_permissions', 'roles', ['role_id'], ['id'])
    op.create_foreign_key('roles_permissions_perm_id_fkey', 'roles_permissions', 'permissions', ['perm_id'], ['id'])
    op.drop_constraint(None, 'roles_permissions', schema='auth', type_='unique')
    op.drop_constraint(None, 'roles', schema='auth', type_='unique')
    op.drop_constraint(None, 'permissions', schema='auth', type_='unique')
    op.drop_constraint(None, 'allowed_device', schema='auth', type_='foreignkey')
    op.create_foreign_key('allowed_device_user_id_fkey', 'allowed_device', 'users', ['user_id'], ['id'])
    op.drop_constraint(None, 'allowed_device', schema='auth', type_='unique')
    op.drop_table('social_account', schema='auth')
    # ### end Alembic commands ###
